...
import { useImageModelConfig, Provider, providerDisplayNames, ImageModelInfo } from "../config/ImageModelConfig";
...
function AddModelInput({ provider, onAdd }: AddModelInputProps) {
    ...
    const placeholders: Record<Provider, string> = {
        openai: "e.g., dall-e-3, dall-e-2",
        gemini: "e.g., imagen-3.0-generate-001",
        openrouter: "e.g., openai/dall-e-3",
    };
...
export function ImageModelSelector() {
    const styles = useStyles();
    const { providers: providerConfigs, selectedModel, setSelectedModel, addModel, removeModel } = useImageModelConfig();

    const handleSelectModel = (model: ImageModelInfo) => {
    ...
    return (
        <Card className={styles.card}>
            <CardHeader header={<Text className={styles.sectionTitle}>Image Model Selection</Text>} />
            ...
            {selectedModel ? (
                ...
                <div className={styles.currentModel}>
                    ...
                    <Text className={styles.currentModelValue}>Using default (DALL-E 3)</Text>
                </div>
            )}
            ...
            <Text className={styles.description}>
                Add image generation models for each provider. The selected model will be used for image generation.
            </Text>
        </Card>
    );
}
